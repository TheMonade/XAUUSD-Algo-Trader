# ARCHIVE — STRAT-001: XAU SMC TRADER (CLOSED — FAILED)

> **File:** `ARCHIVE_STRAT-001_SMC.md` — Tài liệu lịch sử, **ĐÓNG BĂNG**. Không áp dụng cho strategy ACTIVE. Không xóa — tài sản tri thức chống "phát minh lại" (M7, MEMORY.md Mục 7.3).
> **File code tương ứng:** `XAU_SMC_Trader.mq5` — cũng ĐÓNG BĂNG, CẤM chỉnh sửa, chỉ dùng làm kho code nguồn để copy module (bản đồ copy: MEMORY.md Mục 2.3).
> **Ngày đóng băng:** 2026-08-04. **Người ra quyết định đóng:** kết quả backtest 7 năm + chẩn đoán M3 (MEMORY.md Mục 9.1 bản rev 7, chuyển vào đây).

---

## A1. Tổng quan strategy

* **Tên:** XAU SMC Trader — STRAT-001.
* **Phương pháp:** SMC (Smart Money Concepts) đa khung thời gian.
* **Pipeline cuối cùng:** `Session/Kill-zone → HTF Bias (H4/D1) → H1 Order Block (POI) → M5 Liquidity Sweep → M5 CHoCH → M5 FVG → Stops Clamp → Market entry`.
* **Triết lý quản trị:** Risk cố định % balance, quản lý lệnh theo R-multiple, chống overtrade (daily cap / loss-streak cooldown / loss-zone blacklist).
* **Vòng đời:** khởi tạo (H1 OB + M5 CHoCH) → 6 feature merge (P0-001, P1-008, P1-009, P1-010, P1-011, P0-012) → CLOSED 2026-08-04.

## A2. Kết luận & Bằng chứng thất bại

* **Kết luận:** Pipeline SMC **không có edge trên XAUUSD** của broker test.
* **Bằng chứng:** backtest 7 năm (2019.07–2026.07), cấu hình lõi trần (tắt Sweep/FVG/Bias filter — chẩn đoán theo M3): equity giảm đều đặn từ 10,000 → ~8,600 (**−14%**), không pha phục hồi bền; số lệnh rất ít; lệnh thua lớn và liên tiếp. Đường equity đặc trưng "bleed chậm" của strategy bị quét SL liên tục.
* **Chẩn đoán gốc rễ:**
  1. Mean-reversion counter-trend trên tài sản trend mạnh (XAU xuyên thủng OB H1 như "dao qua bơ" trong trending day).
  2. SL neo tại `sweep_extreme` + buffer `0.15 × ATR(M5)` quá hẹp so với volatility XAU → bị quét bởi noise + spread giãn.
  3. Chi phí spread/slippage chiếm tỷ trọng lớn trong SL hẹp M5 → RR thực tế thấp hơn RR danh nghĩa.
  4. Filter stacking (5 lớp gate) bóp tần suất lệnh xuống dưới ngưỡng ý nghĩa thống kê.

## A3. Trạng thái code khi đóng băng

* **File:** `XAU_SMC_Trader.mq5`. **Đã merge:** FEAT-P0-001 (Auto Filling Mode), P1-008 (Liquidity Sweep), P1-009 (FVG Phase 1), P1-010 (HTF Bias), P1-011 (Session Filter), P0-012 (Stops Level Clamp). Không còn nợ P0.
* **Inputs:** 11 nhóm (TRADE / OB / SL BUFFER / CHoCH / SWEEP / FVG / HTF BIAS / SESSION / POSITION MGMT / OVERTRADE / MISC).
* **Cây hàm:** OnInit/OnDeinit, OnTick, OnTradeTransaction, `ResolveSessionWindows`/`IsInTradeSession`/`SessionHourToMin` [P1-011], `ComputeTFBias`/`UpdateHTFBias`/`CheckHTFBiasDirection` [P1-010], `GetOB_H1`, `DetectLiquiditySweep_M5` [P1-008], `DetectCHoCH_M5`, `DetectFVG_M5`/`CheckFVGConfluence` [P1-009], `IsPeak`/`IsTrough`, `HasOpenPosition`/`GetOpenPosition`, `ManagePosition`, `GetMinStopDistance` [P0-012], `CalcLotSize`, `ResolveFillingMode`/`FallbackFillingMode`/`SafeOrderSend` [P0-001], `IsZoneBlocked`/`RegisterLossZone`, `IsMarketTradeable`/`InCooldown`/`UpdateDailyCounterIfNewDay`.

## A4. Chi tiết logic (đóng băng — lưu trữ tham khảo)

### A4.1. HTF Bias Filter H4/D1 [P1-010]

* Phương pháp: cấu trúc Swing (HH/HL, LH/LL) hoặc Giá vs EMA (close[1] vs EMA50 + slope + flat check), hoặc đồng thuận cả hai (BOTH).
* Chế độ đa khung: H4 only / D1 only / H4+D1 đồng thuận / H4 chính (D1 dự phòng khi H4 neutral).
* Cache theo nến HTF (cache key = `iTime(tf, 0)`); chỉ recompute khi có nến HTF mới.
* Neutral Policy: `NEUTRAL_ALLOW_ALL` (default) hoặc `NEUTRAL_BLOCK_ALL`.
* Handle có điều kiện: `UseHTFBiasFilter = false` → không tạo handle H4/D1/EMA nào.
* Swing HTF dùng `IsPeak`/`IsTrough` với `HTF_SwingDistance = 3`, prominence = `ATR(TF) × 0.5`, ATR ref tại index `distance + 1`.

### A4.2. Order Block H1 — `GetOB_H1()`

* Quét `OB_Lookback = 100` nến H1 (từ index 1, bỏ nến đang chạy).
* Bullish OB: nến tăng `body > ATR(15) × 1.5` (displacement), nến ngay trước (`i+1`) giảm → OB = `[low, high]` nến giảm đó. Bearish đối xứng.
* Mitigation check: quét từ nến OB về hiện tại, `close` phá xuyên đáy (bullish)/đỉnh (bearish) → mất hiệu lực.
* Trả OB gần nhất còn hiệu lực (return-first).

### A4.3. CHoCH M5 — `DetectCHoCH_M5()` (bullish; bearish đối xứng)

1. Touch check: trong 100 nến M5 có ≥ 1 nến `low` chạm vùng OB (`bottom ≤ low ≤ top`).
2. Swing high gần nhất qua `IsPeak()`, `Peak_Distance = 5`, prominence `≥ ATR(M5) × 0.5`.
3. Trigger fresh-break: `close[1]` phá lên swing high và `close[2]` còn dưới.
4. Sweep filter [P1-008]: độ xuyên `0.05`–`2.0 × ATR(M5)` qua `ref_level` (swing cũ hoặc mép OB), tuổi sweep ≤ 36 nến; khóa lệnh nếu có nến đóng hẳn qua cực (breakdown).
5. Smart SL: `sweep_extreme` (khi sweep ON) hoặc fallback `lowest low` từ swing peak về hiện tại; SL phải `≤ ob.top`.
6. Output mở rộng [P1-009]: `CHoCHResult` mang `sweep_bar` + `trigger_peak_idx`.

### A4.4. FVG M5 [P1-009, Phase 1 — filter mode]

* Bullish FVG: `low[i] > high[i+2]`; nến giữa (`i+1`) phải bullish displacement, `body > ATR(M5) × FVG_DispATRMult` (1.0). Bearish đối xứng.
* Điều kiện: gap ∈ `[0.10, 3.0] × ATR`; tuổi ≤ 24 nến; chưa mitigated (check theo **close**, không wick); thuộc chân sóng CHoCH (`i+2 ≤ trigger_peak_idx`); tùy chọn overlap OB (default OFF).
* Chọn FVG hợp lệ gần nhất (return-first). Phase 2 (pending limit @ CE) chỉ có spec, không implement.

### A4.5. Session/Kill-zone Filter [P1-011]

* London KZ 09:00–12:00, NY KZ 14:00–17:00 (giờ server broker GMT+2/+3, US DST — ổn định quanh năm theo giờ server); custom window hỗ trợ overnight (22:00–02:00).
* Friday cutoff 20:00; Monday delay (tùy chọn, default OFF); end-buffer chặn entry 15 phút cuối window.
* Time base SERVER (default) / GMT qua `Session_GMT_Offset`; mọi giờ quy về phút-từ-nửa-đêm; bỏ qua giây; log theo transition (không spam theo nến).
* Lưu ý vận hành quan trọng: default giờ chỉ đúng cho broker GMT+2/+3 — broker khác timezone phải chỉnh tay hoặc dùng TIMEBASE_GMT.

### A4.6. Stops Level Clamp [P0-012]

* `GetMinStopDistance()` = `max(STOPS_LEVEL, FREEZE_LEVEL, 2 × spread) × Point + 2 points safety`; đọc động mỗi lần gọi, không cache.
* Entry: clamp SL theo 2 nhánh BUY/SELL riêng; TP/RR/Lot tự tính lại trên `sl_dist` mới; RR sau clamp < MinRR → hủy setup.
* Trailing: guard skip modify khi `new_sl` vi phạm min distance (KHÔNG nới SL — giữ kỷ luật R-multiple); log chống spam qua `g_TrailClampLogged`.
* Hành vi entry = clamp, trailing = skip — khác nhau có chủ đích.

### A4.7. Quản lý lệnh & chống overtrade

* Partial close 66% volume tại lãi ≥ 1.0R (check `lot_min`/`lot_step`, có fallback filling).
* Trailing: kích hoạt từ 1.0R, SL cách giá 1.0R, chỉ modify khi cải thiện > 0.1R + qua clamp P0-012.
* Daily cap 4 lệnh/ngày; 2 lệnh thua liên tiếp → cooldown 12 nến H1 (profit gồm swap + commission).
* Loss-zone: ring buffer 10 slot, block re-entry 24 nến H1, proximity `ATR(H1) × 1.0`.

### A4.8. Execution [P0-001]

* `ResolveFillingMode()` dò bitmask broker: FOK → IOC → RETURN; `SafeOrderSend` retry tối đa 2 lần, fallback filling runtime khi retcode 10030 (kể cả `PositionClosePartial`).
* Deviation 30 points; mọi lệnh qua `CTrade` + Magic 202405; check Symbol + Magic khi lọc position.

## A5. Bài học rút ra (input bắt buộc cho mọi strategy sau)

1. **Khung M5 quá nhỏ cho XAUUSD** — noise/spread nuốt edge; strategy sau tối thiểu M15, ưu tiên H1.
2. **SL phải theo ATR khung lớn (H1+)**, không theo cấu trúc khung nhỏ.
3. **Mọi strategy mới phải giải thích được "vì sao nó hợp với đặc tính trend của XAU"** trước khi viết dòng code đầu tiên.
4. Filter có giá trị thật (Session, Bias) nhưng **không cứu được lõi entry sai** — thứ tự thiết kế đúng: lõi entry trước, filter sau.
5. Hạ tầng (execution, risk, filters) tái sử dụng được ~60–70% — đầu tư vào hạ tầng không bao giờ lãng phí.

## A6. Các phương án thay thế đã phân tích (2026-08-04)

| Phương án | Mô tả | Quyết định |
|---|---|---|
| A. Donchian Breakout H1 + ATR trailing | Trend-following thuần, 3 tham số | **→ STRAT-002 (ACTIVE, xem MEMORY.md)** |
| B. EMA Pullback (H1 trend → M15 entry) | Trend continuation, tần suất cao hơn | Dự phòng #1 |
| C. Session Opening Range Breakout | Breakout đầu London/NY, tận dụng module Session | Dự phòng #2 |
| D. Nới/nâng khung SMC (OB H4 + CHoCH M15) | Cùng logic, đổi timeframe | Loại — lõi trần đã chứng minh không edge, không cứu vãn |

## A7. Lịch sử backtest đáng ghi nhận

| Run | Cấu hình | Kết quả |
|---|---|---|
| SMC full pipeline | Session + Bias + OB + Sweep + CHoCH + FVG | Lỗ, rất ít lệnh sau 7 năm |
| SMC lõi trần (chẩn đoán M3) | Tắt Sweep/FVG/Bias | Equity 10,000 → ~8,600 (−14%), bleed chậm đều đặn → **bằng chứng đóng strategy** |
